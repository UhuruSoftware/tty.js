...
function checkConfig(conf) {
  ...
  conf.shell = conf.shell || process.env.SHELL || 'bash';

  // arguments to shell, if they exist
  var path = require('path');
  var errorShellPath = path.join(__dirname, "../bin/error.sh");
  
  conf.shellArgs = conf.shellArgs || [errorShellPath];

  // commander URL
  conf.commander = conf.commander || 'http://127.0.0.1:8000'
