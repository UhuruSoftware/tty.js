...
function Session(server, socket) {
  ...
  var url = require('url');
  var url_parts = url.parse(this.req.headers.referer, true);
  var query = url_parts.query;
  
  if (query.connectionData != null)
  {
    try
    {    
      var connectionData = JSON.parse(new Buffer(query.connectionData, 'base64').toString('ascii'));
      var request = require('request');
      
      var commanderUrl = conf.commander + "/ssh_config?token=" + connectionData.token;
      this.log('Asking %s for BOSH config.', commanderUrl);
      
      request(commanderUrl, function(error, response, body) {
	if (!error && response.statusCode == 200)
	{
	  self.connectionData = connectionData;
	  self.connectionData.configFile = body;
	  self.log('Commander replied ok.');
	}
	else
	{
	  self.warning('Commander said %s.', response.statusCode);
	}
	
	sessions[self.id] = self;
	self.log('Session \x1b[1m%s\x1b[m created.', self.id);
      });
    }
    catch (err)
    {
      ...
      this.error('Error while trying to decypher connectionData: %s', err.toString());
    }
  }
  else
  {
...
Session.prototype.handleCreate = function(cols, rows, func) {
  ...
  {
    var configFile = self.connectionData.configFile;
    
    this.log('Using BOSH config file %s from commander.', configFile);
    ...
    if (fs.existsSync(configFile))
    {
      ...
      shellArgs[0] = "exec";
      shellArgs[1] = "bosh";
      shellArgs[2] = "-c" + configFile;
      shellArgs[3] = "-d" + self.connectionData.deployment;
      shellArgs[4] = "ssh";
      shellArgs[5] = self.connectionData.job;
      shellArgs[6] = self.connectionData.index;
    }
    else
    {
      this.warning('BOSH config file does not exist.');
    }
  }
  else
  {
    this.warning('No connection data.');
  }
